    ...
    def train(self, schedule=None):
        ...
        if epoch == 1:
            ...
            for i in range(25):
                ...
                msg = time.strftime("[%Y-%m-%d_%H_%M_%S] ", time.localtime()) + "Train Mask loss: {:.4f} | Norm: {:.3f} | Diversity: {:.3f}\n".format(total_loss, loss_norm, loss_div)
                log(msg)
                ...
                if epoch % self.current_schedule['test_epoch_interval'] == 0:
                    msg = time.strftime("[%Y-%m-%d_%H_%M_%S] ", time.localtime()) + "Test Norm: {:.3f} | Diversity: {:.3f}\n".format(loss_norm_eval, loss_div_eval)
                    log(msg)
                epoch += 1
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            msg = time.strftime("[%Y-%m-%d_%H_%M_%S] ", time.localtime()) + "Train CE loss: {:.4f} | BA: {:.3f} | ASR: {:.3f} | Cross Accuracy: {:3f}\n".format(
            ...
            if epoch > self.current_schedule['epochs']:
                ...
                msg = time.strftime("[%Y-%m-%d_%H_%M_%S] ", time.localtime()) + \
